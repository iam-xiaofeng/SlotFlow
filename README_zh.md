...
**[▶ 查看真实对话实录](#)** —— 把这个链接换成你部署后的地址。

一个纯静态、不需要后端的页面，收录 10 段 SlotFlow 的**真实运行**：超长工具结果被卸载后再取回、
压缩之后仍能捞回早期细节、沙箱里真跑代码、HITL 澄清、Skills 两段式加载，以及两个复杂长任务
（一个可以在页面里直接预览成品的落地页，和一次 19 次工具调用的深度调研）。
思考过程、工具时间线、todo 面板、澄清卡片、artifact 预览都能看到。
重新生成或自托管见 [`demo/`](./demo)。
