...
**[▶ 打开在线演示](https://slotflow-demo.pages.dev)** —— 免注册，不用装任何东西。

一个纯静态、不需要后端的站点，**由产品自己的前端渲染**——不是仿的页面，也不是录屏。
它回放了一次真实运行：*「根据 `frontend-design` + `ui-ux-pro-max` 这两个 skill 做一个
高视觉、精美的营销首页」*。模型自己决定了怎么做：两段式加载技能、列 todo 拆解任务、
反问澄清、**并行派出三个子代理**、在 Docker 沙箱里跑代码，最后交付一套
Vite + React 19 + TypeScript 工程——构建产物可以在产物面板里**直接预览**。

页面上每一个字都是录下来的真实输出：约 28.4 万字思维链、29 次工具调用、todo 面板、
澄清卡片、52 个产物。只读，能看不能聊。重新生成或自托管见 [`demo/`](./demo)。
