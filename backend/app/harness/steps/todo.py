"""Step: todo-list reminder injection.

Extracted from ``SlotFlowTodoMiddleware.before_model``. The ``write_todos`` tool exposure and
system prompt stay owned by the thin middleware (it subclasses ``TodoListMiddleware`` which
provides the tool); only the reminder logic is extracted for node reuse.
"""
